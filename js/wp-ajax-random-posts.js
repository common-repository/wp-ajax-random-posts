...
;(function() {
	...
	function get_random_posts(args){
		clearTimeout(timer);
		var url = '?action=wpAjaxRandomPosts&'+args;
        xmlHttp = createxmlHttp();
        xmlHttp.open("GET", url, true);
		xmlHttp.setRequestHeader("Content-type", "charset=UTF-8");
		if($('random-post-more')||0)
			$('random-post-more').innerHTML='<DIV style="background:url('+baseurl+'plugins/wp-ajax-random-posts/img/loading.gif) left center no-repeat;padding-left:20px;'+')" class="ajax-loader">Loading...<p></p></DIV>';
